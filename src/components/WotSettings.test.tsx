...
describe('WotSettings', () => {
  ...
  it('renders the component with initial state', () => {
    ...
    expect(screen.getByText('Trust Level: Normal')).toBeInTheDocument();
  ...
  it('calls setDegrees when the slider is moved', () => {
    const setDegrees = vi.fn();
    mockUseWotStore.mockReturnValueOnce({ ...mockUseWotStore(), setDegrees });
    render(<WotSettings />);
    fireEvent.click(screen.getByText('Strict'));
    expect(setDegrees).toHaveBeenCalledWith(1);
  });
