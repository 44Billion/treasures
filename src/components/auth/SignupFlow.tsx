// Inline-friendly clone of SignupDialog body. This is a near-verbatim copy of
// the dialog's step flow used by HowTo to embed signup in the page rather than
// in a modal. SignupDialog itself remains the source of truth used everywhere
// else; it is marked "stable, do not modify". Keep this file in sync if/when
// the signup steps change.
...
interface SignupFlowProps {
  /** Called once the user has finished (or skipped) the profile step. */
  onComplete?: () => void;
}

/**
 * Inline signup flow: welcome → forge → secure → profile. Behaves like
 * SignupDialog without the modal chrome. Designed to live inside a chapter or
...
export function SignupFlow({ onComplete }: SignupFlowProps) {
  ...
  // Clear any stale "fresh signup" marker on mount so LoginArea's fallback
  // WelcomeModal trigger does not fire while the inline flow is running.
  useEffect(() => {
    localStorage.removeItem('treasures_last_signup');
  }, []);

  // Autofocus display name when reaching profile step.
  useEffect(() => {
    if (step === 'profile') {
  ...
  const finishSignup = async (skipProfile = false) => {
    // NOTE: deliberately NOT writing `treasures_last_signup` here. That
    // localStorage marker is a fallback used by LoginArea's WelcomeModal
    // trigger to detect a fresh dialog-driven signup. In the inline flow we
    // have a direct onComplete callback and don't want the global welcome
    // modal popping up on top of the HowTo page mid-journey.
  ...
  // Step-aware surface background. Mirrors the dialog gradients so the inline
  // flow visually matches the modal version users see elsewhere.
  const surfaceClass =
    step === 'welcome'
  ...
  const descriptionNode =
    step === 'welcome' ? t('signup.dialog.welcome.description')
    : step === 'generate' ? t('signup.dialog.generate.description')
    : step === 'download' ? <>{t('signup.dialog.download.descriptionLine1')}<br />{t('signup.dialog.download.descriptionLine2')}</>
    : step === 'profile' ? t('signup.dialog.profile.legendStarts')
    : t('signup.dialog.done.description');

  return (
    <div className={`relative overflow-hidden rounded-2xl border border-border/40 text-foreground ${surfaceClass}`}>
      {/* Header */}
      <div className='px-6 pt-6 pb-1 text-center relative z-10'>
        <div className='font-semibold text-lg'>{titleNode}</div>
        <div className='text-sm text-muted-foreground mt-1'>{descriptionNode}</div>
      </div>

      {/* Watermark logo on welcome */}
      {step === 'welcome' && (
        <img
          src='/icon.svg'
          alt=''
      ...
      <div className='px-6 pt-2 pb-5 space-y-4 relative z-10'>
        {/* ── Welcome ── */}
        {step === 'welcome' && (
          ...
          <div className='space-y-3'>
            ...
            <div className={`space-y-3 text-left ${isPublishing ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className='space-y-1.5'>
                <label htmlFor='profile-name-inline' className='text-sm font-medium flex items-center gap-1.5'>
              ...
              <div className='space-y-1.5'>
                <label htmlFor='profile-about-inline' className='text-sm font-medium flex items-center gap-1.5'>
                  <ScrollText className='w-3.5 h-3.5 text-muted-foreground' />
                  {t('signup.dialog.profile.bio')}
                </label>
                <Input
                  id='profile-about-inline'
              ...
              <div className='space-y-1.5'>
                <label htmlFor='profile-picture-inline' className='text-sm font-medium flex items-center gap-1.5'>
                  <Camera className='w-3.5 h-3.5 text-muted-foreground' />
                  {t('signup.dialog.profile.avatar')}
                </label>
                <div className='flex gap-2'>
                  <Input
                    id='profile-picture-inline'
