  ...
  return (
    <div className="adventure-popup-card w-[min(320px,calc(100vw-4rem))] overflow-hidden">
      ...
      <div className="relative cursor-pointer" onClick={handleView}>
        ...
        <div className={`absolute bottom-0 left-0 right-0 px-3 pb-1.5 ${hasImage ? '[text-shadow:0_2px_8px_rgba(0,0,0,0.7),0_1px_2px_rgba(0,0,0,0.9)]' : ''}`}>
          <h3 className={`font-semibold text-sm leading-snug line-clamp-2 ${hasImage ? 'text-white' : 'text-foreground'}`}>
            {adventure.title}
          </h3>
          <div className={`flex items-center gap-1.5 text-[11px] mt-0.5 ${hasImage ? 'text-white' : 'text-muted-foreground'}`}>
            {authorPicture && (
              <img
                src={authorPicture}
                alt=""
                className="h-4 w-4 rounded-full object-cover flex-shrink-0 ring-1 ring-white/30"
              />
            )}
          ...
          {(adventure.summary || adventure.description) && (
            <p className={`text-[11px] mt-0.5 line-clamp-2 leading-snug ${hasImage ? 'text-white/95' : 'text-muted-foreground'}`}>
              {adventure.summary || adventure.description}
            </p>
          )}
        </div>
