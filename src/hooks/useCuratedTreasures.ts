...
/**
 * Map from `pubkey:dTag` to a local hero image path (e.g. "/hero/curated-0.webp").
 * Images are generated by `scripts/fetch-hero-images.js` into `public/hero/`.
 * The index matches the order of CURATED_NADDRS / DECODED_CURATED.
 */
const HERO_IMAGE_MAP = new Map<string, string>(
  DECODED_CURATED.map((item, i) => [`${item.pubkey}:${item.identifier}`, `/hero/curated-${i}.webp`]),
);

/** Get the local hero image path for a curated treasure, if available. */
export function getCuratedHeroImage(pubkey: string, dTag: string): string | undefined {
