...
export function useUploadFile() {
  ...
  return useMutation({
    mutationFn: async (file: File) => {
      ...
      let uploadFile = file;
      if (config.imageQuality === 'compressed' && file.type.startsWith('image/')) {
      ...
      const uploader = new BlossomUploader({
        servers,
        signer: user.signer,
        // Custom fetch with a 30-second per-server timeout.
        // Promise.any() still resolves as soon as any server succeeds.
