...
//
// This acts as a local "backup relay": every event the app sees from the real
// relays is mirrored into IndexedDB so it can be read back without a network
// round-trip (and survives reloads / offline).
//
// It is NOT a general-purpose relay store. It implements the full `NStore`
// interface, but only understands two filter shapes:
//
//   1. ID filters:    { "ids": [A, B, C] }
//   2. Addr filters:  { "kinds": [0], "authors": [pubkey] }
//                     { "kinds": [37516], "authors": [pubkey], "#d": ["my-cache"] }
//
...
// and "give me this author's latest kind 0 / addressable event", not a full
// index. Tag-filtered queries (e.g. logs by #a coordinate) still go to relays.
//
// Storage layout:
...
export interface NIndexedDBStoreOpts {
  /** Database name. Defaults to `treasures-events`. */
  name?: string;
...
export class NIndexedDBStore implements NStore {
  ...
  /** Whether a flush is already scheduled, so we only schedule once per burst. */
