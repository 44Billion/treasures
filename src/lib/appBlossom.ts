...
/** Normalize a Blossom server URL for deduplication (lowercase, strip trailing slash). */
 ...
 * server list, and can be optionally combined with user servers.
 */
 ...
 *
 * Order: app servers first (when enabled), then user servers.
 * Matches BUD-03's "most trusted first" ordering convention.
 */
export function getEffectiveBlossomServers(
  userMeta: BlossomServerMetadata | undefined,
  useAppBlossomServers: boolean,
): string[] {
  if (!userMeta?.servers || !Array.isArray(userMeta.servers)) {
