...
export function invoiceCommitsTo(decoded: DecodedInvoice, candidates: string[]): boolean {
  ...
  // Use noble's `utf8ToBytes` rather than a bare `TextEncoder`: it re-wraps the
  // result in a same-realm `Uint8Array`, which keeps noble's `instanceof`
  // check happy under the jsdom test environment (a raw `TextEncoder` there
  // yields a cross-realm typed array that noble rejects).
  return candidates.some(
    (candidate) => bytesToHex(sha256(utf8ToBytes(candidate))) === decoded.descriptionHash,
  );
}
