...
export class NostrBatcher {
  ...
  /**
   * Optional local cache ("backup relay"). Every event that flows out of
   * `.query()` / `.req()` is written here so the rest of the app can read it
   * back cache-first (and offline). The store is a promise because IndexedDB
   * opens asynchronously; we never block a relay read on it.
   */
