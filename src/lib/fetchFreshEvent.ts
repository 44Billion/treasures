 ...
 * from the last event we actually observed instead of an empty base. The newer
...
export async function fetchFreshEvent(
  ...
  const timeout = AbortSignal.timeout(TIMEOUTS.QUERY);
  ...
  // an event older than the one we already have.
