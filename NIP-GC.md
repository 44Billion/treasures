...
- `attestation` (optional) - cryptographic signature attesting to a verified find

## Verified Finds

The log event can include an attestation signature to cryptographically verify that the cache was actually found. This signature is created by the cache's verification key and proves that the specific finder located the specific cache.

The attestation signature is generated by signing the string `<finder-npub>:<cache-naddr>` with the cache's verification private key, where:
- `finder-npub` is the log author's public key in npub format  
- `cache-naddr` is the cache's Nostr address (naddr format)

This approach creates a direct cryptographic link between the finder and the cache that cannot be forged or transferred.
...
```json
{
  ...
  "tags": [
    ["a", "37515:0461fcbecc4c3374439932d6b8f11269ccdb7cc973ad7a50ae362db135a474dd:first-treasure-1748619568668"],
    ["log-type", "found"],
    ["attestation", "a1b2c3d4e5f6789012345678901234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef12345678"]
  ]
}
```